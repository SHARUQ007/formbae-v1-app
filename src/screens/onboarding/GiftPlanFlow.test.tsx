...
it('a plan for one person goes straight to checkout', async () => {
  await render(paywall());
  const soloCard = renderer.root.findAllByProps({ accessibilityRole: 'radio' }).find(node => node.props.accessibilityState?.selected === false)!;
  await act(async () => { soloCard.props.onPress(); });
  expect(cta().props.title).toBe('Pay ₹59 & continue');
  await act(async () => { await cta().props.onPress(); });
